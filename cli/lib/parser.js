...
const log = debug('resultados:parser');

export function parseResults(html) {
  const results = [];

  log('Starting HTML parsing, content length: %d', html.length);
  ...
  if (!largeTableMatch) {
    log('No results table found in HTML');
    return results;
  }
  
  const tableHtml = largeTableMatch[1];
  log('Results table found, processing %d bytes', tableHtml.length);

  // Extract all table rows
  const rowMatches = [...tableHtml.matchAll(PATTERNS.TABLE_ROW)];
  log('Table rows found: %d', rowMatches.length);
  ...
  for (let i = 0; i < rowMatches.length; i++) {
    ...
    if (pdfLinkMatch) {
      ...
      if (cells.length >= 4) {
        ...
        if (result.order && result.license && result.transmitted && result.pdfUrl) {
          results.push(result);
        } else {
          log('Row skipped: missing data - order=%s, license=%s, transmitted=%s, pdfUrl=%s', 
              !!result.order, !!result.license, !!result.transmitted, !!result.pdfUrl);
        }
      } else {
        log('Row skipped: insufficient cells (%d)', cells.length);
      }
    }
  }

  log('Total results parsed: %d', results.length);
  return results;
}
...
export function displayResultsTable(results, options = {}) {
  ...
  if (options.sessionId) {
    ...
    console.log(pc.gray('💡 Copia y pega estos comandos en tu terminal para descargar los PDFs'));
    console.log(pc.gray('💡 O usa el comando "download" para descargarlos automáticamente'));
  }
}